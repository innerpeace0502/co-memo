...
import type { Memo, MemoColor, UpdateMemoInput } from '@/lib/types';

interface EditMemoModalProps {
  memo: Memo | null;
  onClose: () => void;
  onSubmit: (id: string, input: UpdateMemoInput) => Promise<void>;
}

export default function EditMemoModal({ memo, onClose, onSubmit }: EditMemoModalProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setSaving(true);
    try {
      await onSubmit(memo.id, { title: title.trim(), content: content.trim(), color });
      onClose();
    } finally {
      setSaving(false);
    }
  ...
  return (
    <Modal isOpen={!!memo} onClose={onClose} title="메모 수정">
